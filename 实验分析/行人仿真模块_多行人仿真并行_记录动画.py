...
class PedestrianEnv(gym.Env):
    ...
    def render(self, mode="human"):
        ...
        pos = {n: n for n in self.G.nodes()}  # 把图中每个节点 n 本身（通常是坐标元组 (x, y)）作为它在绘图时的位置 pos[n]。
        if not pos:
            pos = nx.spring_layout(self.G)

        edges = list(self.G.edges())
        # counts = np.array([self.G[u][v].get("count", 1) for u, v in edges])
        # widths = counts / counts.max() * 5
        # colors = counts
        cmap = plt.cm.Reds

        nx.draw_networkx_nodes(self.G, pos, node_size=100, node_color="lightgray", ax=ax)
        ...
        if hasattr(self, 'current'):
            nx.draw_networkx_nodes(self.G, pos, nodelist=[self.current], node_color='yellow', node_size=300, ax=ax)
...
def zhixingfangzhen(index1,distance_matrix,road_shp, point_shp,speed,max_time,sumPeople):
    ...
    def simulate_one_person(speed, max_time, max_deg):
        ...
        path = []
        # 保存初始帧
        frame = env.render()  # 确保 render() 返回 RGB 图像
        frames.append(frame)
        while not done:
        ...
        env.close()
        # 保存为 GIF（可换成 .mp4）
        imageio.mimsave("pedestrian_simulation.gif", frames, fps=5)
        return path
    ...
    # )
    all_paths = [simulate_one_person(speed, max_time, 4)]
    ...
    if values:
        ...
        plt.axis("off")
        plt.tight_layout()
        plt.savefig(f"./分析结果/图片/{index1}.png")
        plt.show()
        # 生成csv
        # 遍历图中的节点，提取属性
        records = []
        ...
        # 保存为 CSV
        df.to_csv(f"./分析结果/表/{index1}_{round(r2,4)}.csv", index=False, encoding='utf-8-sig')
        return r2
    else:
        print("没有有效数据。")
        return "没有有效数据。"
